...
pub async fn greeting_script(app_handle: AppHandle) -> Result<String, String> {
    ...
    println!("🟣 greeting_script: Asking for time of day");
    let time_of_day = kit.ask_select(
        "When do you feel most productive?",
        vec!["Morning", "Afternoon", "Evening", "Night"]
    ).await?;
    println!("🟣 greeting_script: Received time of day: {}", time_of_day);
    ...
    println!("🟣 greeting_script: Showing personalized HTML");
    // Reset awaiting flag so the HTML stays visible after completion
    kit.reset_awaiting_flag();
    kit.render_html("Your Personalized Greeting", &output_html)?;

    // Signal that the script is complete - HTML will stay visible since awaiting flag was reset
    println!("🟣 greeting_script: Signaling script completion - HTML should remain visible");
    kit.script_complete().await?;

    let result = format!("Greeting created for {} (age: {}, hobby: {})", name, age as i32, hobby);
    println!("🟣 greeting_script: Returning result: {}", result);
    Ok(result)
}
