# SnapRun - Production Deployment Guide

## Overview
SnapRun is a Windows automation platform built with Tauri, featuring a Rhai scripting engine and modern React UI with glass effects.
...
1. Download the installer:
   - **MSI Installer**: `SnapRun-1.0.0.msi` (Windows Installer)
   - **NSIS Installer**: `SnapRun-1.0.0.exe` (Custom installer)
...
1. Download `SnapRun.exe` from the release
2. Create a folder for SnapRun
3. Run `setup_SnapRun.bat` for configuration

## Configuration 🔧

### Environment Variables
SnapRun supports the following environment variables:

- **SnapRun_HOME**: Main application directory
  ```
  Default: %USERPROFILE%\Documents\SnapRun
  ```

- **SnapRun_SCRIPTS**: Scripts directory
  ```
  Default: %SnapRun_HOME%\Scripts
  ```
...
### Starting SnapRun
- **Desktop Shortcut**: Double-click SnapRun icon
- **Start Menu**: Search for "SnapRun"
- **Command Line**: Run `SnapRun.exe`
- **System Tray**: Look for SnapRun icon in system tray
...
### Built-in Functions
SnapRun provides extensive APIs for script development:
...
1. Create a `.rhai` file in `%SnapRun_SCRIPTS%\custom_scripts\`
2. Use SnapRun APIs and standard Rhai syntax
...
write_file(demo_file, "This is a demo file created by SnapRun!");
...
### Build Output
- **Executable**: `src-tauri/target/release/SnapRun.exe`
